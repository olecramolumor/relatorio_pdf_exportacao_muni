...
def main(tipo_aba,nome_arquivo, ano,logo):
    ...
    try:
        ...
        df_dados,df_balanca=relatorio_dataframe.main(tipo_aba,ano,pais)

        logger.info("Coletando Dados de Exportação e Importação")
        df_exp = gfs.dataframe_exportacao(df_dados)
        df_imp = gfs.dataframe_importacao(df_dados)

        logger.info("Gerando Relatório")
        gerar_relatorio(nome_arquivo, df_exp, df_imp, ano,logo)
        sucesso = True
        logger.info("Relatório Criado Com Êxito!")
