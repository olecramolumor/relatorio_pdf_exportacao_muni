...
#In[5]
def desenhar_cabecalho_rodape(muni,ano,canvas,doc, exibir_imagens=True, caminhos_imagens=None):
    canvas.saveState()
    ...
    #TÍTULO DO RELATÓRIO:
    texto_relatorio = f"Relatório de Comércio Exterior: {muni} - {ano}"
...
def construir_secao_fluxo(story, df_fluxo, tipo_fluxo, styles):
    ...
    img_mun = gfs.gerar_grafico_barras(df_mun, 'pais', 'valor_fob', f"Top 10 Paises por Valor FOB - {tipo_fluxo}")
    story.append(img_mun)
    story.append(NextPageTemplate("Retrato"))
    story.append(PageBreak())

#In[5]:
def gerar_relatorio(nome_arquivo, df_exp, df_imp, muni, ano,logo):
    try:
        ...
        cabecalho_e_rodape_dados = partial(desenhar_cabecalho_rodape, muni, ano, exibir_imagens=logo, caminhos_imagens=arq_cam)
...
#In[]:
def main(tipo_aba,nome_arquivo, ano, muni,logo):
    try:
        df=relatorio_dataframe.main(tipo_aba,ano,muni)
        df_exp = dataframe_exportacao(df)
        df_imp = dataframe_importacao(df)
        gerar_relatorio(nome_arquivo, df_exp, df_imp, muni, ano,logo)
...
if __name__ == "__main__":
    ...
    for logo in logos:
        ...
        muni= "Porto Velho"
        ano = 2025
        nome_arquivo = f"Teste XX - {muni} - {logo}"
        tipo_aba = "pais_ano"
        main(tipo_aba,nome_arquivo, ano, muni,logo)