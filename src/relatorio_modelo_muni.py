...
#In[5]
def desenhar_cabecalho_rodape(municipio,ano,canvas,doc, exibir_imagens=True, caminhos_imagens=None):
    canvas.saveState()
    ...
    #TÍTULO DO RELATÓRIO:
    texto_relatorio = f"Relatório de Comércio Exterior: {municipio} - {ano}"
...
def construir_secao_fluxo(story, df_fluxo, tipo_fluxo, styles):
    ...
    df_mun = df_fluxo.groupby('pais', as_index=False)['valor_fob'].sum().sort_values('valor_fob', ascending=False).head(10)
    img_mun = gfs.gerar_grafico_barras(df_mun, 'pais', 'valor_fob', f"Top 10 Países por Valor FOB - {tipo_fluxo}")
...
#In[5]:
def gerar_relatorio(nome_arquivo, df_exp, df_imp, municipio, ano,logo):
    try:
        ...
        cabecalho_e_rodape_dados = partial(desenhar_cabecalho_rodape, municipio, ano, exibir_imagens=logo, caminhos_imagens=arq_cam)
...
#In[]:
def main(tipo_aba,nome_arquivo, ano, municipio,logo):
    try:
        df=relatorio_dataframe.main(tipo_aba,ano,municipio)
        df_exp = dataframe_exportacao(df)
        df_imp = dataframe_importacao(df)
        gerar_relatorio(nome_arquivo, df_exp, df_imp, municipio, ano,logo)
...
if __name__ == "__main__":
    ...
    for logo in logos:
        ...
        ano = 2025
        nome_arquivo = f"Teste XX - municipio - {logo}"
        tipo_aba = "municipio_ano"
        main(tipo_aba,nome_arquivo, ano, municipio,logo)